  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="text-zinc-400 text-sm">{passed}/{total} checks passed</span>
      </div>
      ...
      <ul className="space-y-1.5">
        {results.map((r, i) => (
          <li key={i} className="flex items-start gap-2 text-sm">
            <span className={`mt-0.5 ${r.passed ? 'text-green-400' : 'text-zinc-600'}`}>
              {r.passed ? '\u2713' : '\u2717'}
            </span>
            <span className={r.passed ? 'text-zinc-300' : 'text-zinc-500'}>
              {r.label}
            </span>
          </li>
        ))}
